...
// ---------- Config - update these ----------
const VISITOR_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbw3ojAieEmKtZo149A_kvr9zuVxjYdV4RMvG_IBxgksDzI0oeDAAj7wJ3AcspUmkEM/exec"; // apps script "exec" URL
const FRONTEND_SECRET = "s3cret-abc-12345"; // secret token (optional but recommended)
// -------------------------------------------
...
async function submitVisitorForm(e) {
  ...
  const honeypot = document.getElementById('visitor-hp').value.trim();
  ...
  // prevent super long submissions
  if (message.length > 4000) {
    statusEl.textContent = "Message is too long.";
    return;
  }

  // If honeypot filled (bot) — do not submit
  if (honeypot !== "") {
    statusEl.textContent = "Detected as spam; message not sent.";
    return;
  }
  ...
  try {
    const response = await fetch(VISITOR_SCRIPT_URL, {
      method: "POST",
    ...
    if (response.ok && result && result.result === "success") {
      ...
      statusEl.textContent = "Unexpected error sending message.";
    }
  } catch (err) {
